...
class TrafficOptimizer:
    ...
    async def _analyze_nodes(self, traffic_data: list[Traffic]) -> dict[str, list[Traffic]]:
        ...
        for node_type, nodes in nodes_by_type.items():
            if nodes:
                ...
                logger.info(
                    f"Тип узла {node_type}: {len(nodes)} узлов, средняя утилизация: {avg_utilization:.2f}, "
    ...
    async def _generate_load_balancing_recommendations(
        self,
        nodes_by_type: dict[str, list[Traffic]],
    ) -> list[dict]:
        ...
        for nodes in nodes_by_type.values():
            if not nodes:
                continue
            node_stats = [(t.node_id, t.bandwidth / t.capacity_mbps, self._calculate_node_weight(t), t) for t in nodes]
            overloaded = [
            ...
            for overload_node, overload_util, _, node_data in overloaded:
                if underloaded:
                    ...
                    available_capacity = (
                        self.utilization_threshold_high - underload_util
                    ) * underload_data.capacity_mbps
    ...
    async def _generate_capacity_planning_recommendations(
        ...
        for node_type, nodes in nodes_by_type.items():
            ...
            high_utilization_nodes = [
                node for node in nodes if node.bandwidth / node.capacity_mbps > self.utilization_threshold_high * 0.9
            ]
            if high_utilization_nodes and len(high_utilization_nodes) / len(nodes) > 0.3:
                avg_utilization = sum(n.bandwidth / n.capacity_mbps for n in high_utilization_nodes) / len(
                    high_utilization_nodes,
                )
    ...
    async def _calculate_network_status(self, traffic_data: list) -> dict:
        ...
        overloaded_nodes = len(
            [t for t in traffic_data if t.bandwidth / t.capacity_mbps > self.utilization_threshold_high],
        )
        high_latency_nodes = len([t for t in traffic_data if t.latency > self.latency_thresholds[t.node_type]])
                ...
                recommendations.append(
                    {
                        ...
                        "backup_nodes": [
                            n.node_id for type_name in secondary_types for n in nodes_by_type.get(type_name, [])
                        ],
    ...
    async def apply_optimization(self, recommendation: dict) -> dict:
        ...
        return {
            "status": "error",
            "message": f"Ошибка применения оптимизации: {e!s}",
            "timestamp": datetime.now(UTC).isoformat(),
        }
    ...
    async def manual_route_switch(self, from_node: str, to_node: str, traffic_percentage: float) -> dict:
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"Ошибка при переключении трафика: {e!s}",
                "timestamp": datetime.now(UTC).isoformat(),
            }
    ...
    async def configure_qos_policies(self, node_id: str, traffic_type: str, priority: int) -> dict:
        ...
        try:
            query = select(Traffic).filter(Traffic.node_id == node_id).order_by(Traffic.timestamp.desc()).limit(1)
            ...
            qos_query = select(QoSPolicy).filter(
            ...
            return {
                "status": "error",
                "message": f"Ошибка настройки QoS: {e!s}",
                "timestamp": datetime.now(UTC).isoformat(),
            }
    ...
    async def update_optimization_statuses(self) -> dict:
        ...
        try:
            ...
            expired_query = select(OptimizationAction).filter(
                OptimizationAction.is_active is True,
        ...
        try:
            ...
            if not traffic_data:
                return {
                    ...
                    "active_optimizations": active_optimizations[:5]
                    if len(active_optimizations) > 5
                    else active_optimizations,
                }
            ...
            for node_type, nodes in node_groups.items():
                ...
                nodes_status[node_type] = {
                    ...
                    "overloaded_nodes": sum(
                        1 for n in nodes if n.bandwidth / n.capacity_mbps > self.utilization_threshold_high
                    ),
                    "underloaded_nodes": sum(
                        1 for n in nodes if n.bandwidth / n.capacity_mbps < self.utilization_threshold_low
                    ),
                }
            ...
            needs_optimization = (
                network_status["status"] == "critical"
                or avg_utilization > self.utilization_threshold_high
                or any(ns["overloaded_nodes"] > 0 for ns in nodes_status.values())
            )
            ...
            return {
                "status": "error",
                "message": f"Ошибка при получении статуса оптимизации: {e!s}",
                "timestamp": datetime.now(UTC).isoformat(),
            }
