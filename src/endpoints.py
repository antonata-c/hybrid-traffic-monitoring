...
from schemas import OptimizationResponse, QoSConfigRequest, SwitchRoutesRequest
...
async def optimize(db: Annotated[AsyncSession, Depends(get_db)]):
...
async def apply_optimization(recommendation_id: int, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    try:
        ...
        return await optimizer.apply_optimization(recommendations["recommendations"][recommendation_id])
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/optimize/active", response_model=list)
async def get_active_optimizations(db: Annotated[AsyncSession, Depends(get_db)]):
...
async def switch_routes(request: SwitchRoutesRequest, db: Annotated[AsyncSession, Depends(get_db)]):
    """Ручное переключение маршрутов между узлами гибридной сети."""
    try:
        optimizer = TrafficOptimizer(db)
        return await optimizer.manual_route_switch(
...
async def configure_qos(request: QoSConfigRequest, db: Annotated[AsyncSession, Depends(get_db)]):
    """Настройка параметров QoS для приоритизации трафика."""
    try:
        optimizer = TrafficOptimizer(db)
        return await optimizer.configure_qos_policies(
...
async def get_optimization_status(db: Annotated[AsyncSession, Depends(get_db)]):
...
async def get_network_topology(db: Annotated[AsyncSession, Depends(get_db)]):
...
async def update_optimization_statuses(db: Annotated[AsyncSession, Depends(get_db)]):
    """Обновляет статусы оптимизаций: деактивирует истекшие и добавляет новые проблемы."""
    try:
        optimizer = TrafficOptimizer(db)
        return await optimizer.update_optimization_statuses()
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
