...
class DatasetDataSource(DataSource):
    ...
    def _initialize_node_mappings(self, nodes: List[Dict]) -> None:
        ...
        self.node_to_router = {
            node["node_id"]: f"router_{i % 12}" 
            for i, node in enumerate(nodes)
        }
        
        self.node_distances = {
            node["node_id"]: np.random.randint(10, 1000) 
            for node in nodes
        }
        
        self.node_status = {
            node["node_id"]: {"available": True, "load_factor": 1.0} 
            for node in nodes
        }
    ...
    def _calculate_route_score(
        ...
        weights = weight_factors.get(node_type, {"latency": 0.25, "utilization": 0.25, "distance": 0.25, "load": 0.15, "noise": 0.1})
    ...
    def _simulate_switch(
        ...
        if (utilization > thresholds_for_type["utilization"] or 
                latency > thresholds_for_type["latency"] or 
                not self.node_status[node_id]["available"] or 
                random_trigger):
            
            alternative_nodes = [(n_id, n_type) for n_id, n_type in self.node_type_map.items() 
